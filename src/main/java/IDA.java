...
public class IDA implements Solver {
	...
	public SearchNode expand(SearchNode node) {
		...
		if (node.state.getBlankX() != 0) {
			...
			left.moveLeft();
			SearchNode leftNode = new SearchNode(left, node, 0, Solver.calcManhattanDistance(left) + node.depth + 1);
		...
		if (node.state.getBlankX() != node.state.N - 1) {
			...
			right.moveRight();
			SearchNode rightNode = new SearchNode(right, node, 1, Solver.calcManhattanDistance(right) + node.depth + 1);
		...
		if (node.state.getBlankY() != 0) {
			...
			up.moveUp();
			SearchNode upNode = new SearchNode(up, node, 2, Solver.calcManhattanDistance(up) + node.depth + 1);
		...
		if (node.state.getBlankY() != node.state.N - 1) {
			...
			down.moveDown();
			SearchNode downNode = new SearchNode(down, node, 3, Solver.calcManhattanDistance(down) + node.depth + 1);
